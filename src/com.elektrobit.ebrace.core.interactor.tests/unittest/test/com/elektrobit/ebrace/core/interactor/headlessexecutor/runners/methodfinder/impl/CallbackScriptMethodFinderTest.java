...
public class CallbackScriptMethodFinderTest
{
    @Test
    public void callbackMethodsDelegatedToGenericMethodFinder() throws Exception
    {
        List<RaceScriptMethod> callbackMethods = Arrays.asList( mock( RaceScriptMethod.class ) );

        RaceScriptInfo script = mock( RaceScriptInfo.class );
        when( script.getCallbackMethods() ).thenReturn( callbackMethods );

        ScriptMethodFinder genericScriptMethodFinder = mock( ScriptMethodFinder.class );

        new CallbackScriptRunPart( mock( ScriptExecutorService.class ), genericScriptMethodFinder )
                .extractScriptMethod( script, "execute" );

        verify( genericScriptMethodFinder ).extractScriptMethod( callbackMethods, "execute" );
