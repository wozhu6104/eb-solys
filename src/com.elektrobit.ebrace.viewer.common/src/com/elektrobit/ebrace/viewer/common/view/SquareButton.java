...
/**
 * The SquareButton class is a simple SWT widget that can be used in place of a native SWT button. Unlike the native
 * button, this should look exactly the same on all platforms, which is very helpful when designing tightly spaced
 ...
 * </code>
 * <p>
 * The default button style looks sort of similar to the buttons on the Gmail web interface. You can very easily create
 * your own styles that look different. Colors can be specified for when the button is unselected, inactive, focused,
 * clicked, and hovered over. If two different background colors are specified for a given button style, the button
 * background will be a top-to-bottom blended gradient of the two colors.
 * <p>
 * You can also specify a background image to be used, either cropped, centered, stretched, tiled, or set such that the
 * button is exactly the size of the background image.
 * <p>
 * This code may be used under the terms of the Apache license, version 2.0. Copyright 2009-2010 Strategic Network
 * Applications, Inc. (http://www.snapps.com).
 * <p>
 * 
...
public class SquareButton extends Canvas
{
    ...
    private void paintControl(PaintEvent e)
    {
        ...
        if (roundedCorners)
        {
            gc.drawRoundRectangle( rect.x + (bw - 1),
                                   rect.y + (bw - 1),
                                   rect.width - bw,
                                   rect.height - 2,
                                   arcWidth,
     ...
     * then button image and text. So if the background image has transparency, the background color will show through
     * the transparency.
     */
    @Override
     ...
     * button. Any part of the image that is too wide or too tall to fit inside the button area is clipped (cropped)
     * off.</li>
     ...
     * <li>BG_IMAGE_CENTER: the center of the image is placed inside the center of the button. Any part of the image
     * that is too tall or too wide to fit will be clipped.</li>
     * <li>BG_IMAGE_FIT: the button will be the exact size of the image. Note that this can sometimes truncate the text
    ...
    /**
     * Set whether or not this button is enabled (active) or not (inactive). This setting can be changed dynamically
     * after the button has been drawn.
     * <p>
     * An inactive button does not change color when it is hovered over or clicked, does not receive focus or
     * participate in the tab order of the widget container, and does not notify listeners when clicked.
     */
    @Override
    ...
    /**
     * Set the inner margin between the left and right of the text inside the button and the button borders, in pixels.
     * Like the left and right padding for the text. Default is 8 pixels.
     * 
    ...
    /**
     * Set the inner margin between the top and bottom of the text inside the button and the button borders, in pixels.
     * Like the top and bottom padding for the text. Default is 4 pixels.
     * 
    ...
    /**
     * Set whether or not a dotted-line border should be drawn around the text inside the button when the button has tab
     * focus. Default is false (no selection border). If a selection border is used, it will be the same color as the
     * font color. Note that you can also use setSelectedColors() to change the look of the button when it has focus.
     * 
     ...
     * @param bgColor2
     *            the gradient color at the bottom of the button (if you don't want a gradient, set this to bgColor1)
     * @param bdrColor
     *            the color of the border around the button (if you don't want a border, use getBackground())
     * @param fntColor
