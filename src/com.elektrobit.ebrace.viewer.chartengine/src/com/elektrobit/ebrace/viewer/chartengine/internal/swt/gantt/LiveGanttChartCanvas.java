...
public class LiveGanttChartCanvas<T> extends BaseChartCanvas<T>
{
    public LiveGanttChartCanvas(Set<RuntimeEventChannel<T>> channels, Composite parent, int style, ChartModel model)
    {
        super( channels, "Live Gantt Chart", parent, style, model );
    }

    @Override
    protected ChartBuilder<T> createChartBuilder(String chartTitle)
    {
        return new GanttChartBuilder<T>( modelToDisplay, RunMode.LIVE ).addChartTitle( chartTitle );
    }
