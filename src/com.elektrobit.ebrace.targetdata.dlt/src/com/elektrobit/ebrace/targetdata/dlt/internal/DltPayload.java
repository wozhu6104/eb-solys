...
public class DltPayload
{
    ...
    private final BytesFromStreamReader bytesReader;

    public DltPayload(BytesFromStreamReader bytesReader)
    {
    ...
    {
        remainingLength -= 1;
        return bytesReader.readNBytes( 1 )[0];
    }

    private short readShort() throws IOException
    {
        SHORT_BUFFER = bytesReader.readNBytes( 2 );
    ...
    {
        INT_BUFFER = bytesReader.readNBytes( 4 );
    ...
    {
        LONG_BUFFER = bytesReader.readNBytes( 8 );
    ...
    {
        FLOAT_BUFFER = bytesReader.readNBytes( 4 );
    ...
    {
        DOUBLE_BUFFER = bytesReader.readNBytes( 8 );
    ...
    {
        byte[] _raw = bytesReader.readNBytes( readShort() );
    ...
    private String readVari() throws IOException
    {
        ...
        {
            paramName = bytesReader.readNBytes( paramNameLength );
            remainingLength -= paramNameLength;
        }
        return new String( paramName );
    }

    private byte[] skip(int bytesToSkip) throws IOException
    {
        byte[] _raw = bytesReader.readNBytes( bytesToSkip );
        remainingLength -= _raw.length;
        return _raw;
    }
