...
public class TimeGraphControl extends TimeGraphBaseControl
    ...
    private int fTopPadding = 0;

    private boolean fTimeSelectionEnabled = true;
    ...
    void paint(Rectangle bounds, PaintEvent e) {
        ...
        Rectangle boundsWithPadding = new Rectangle(bounds.x, bounds.y+fTopPadding, bounds.width, bounds.height-fTopPadding);

        // draw the background layer
        drawBackground(bounds, boundsWithPadding, nameSpace, gc);
        ...
        // draw the items
        drawItems(boundsWithPadding, fTimeProvider, fItemData.fExpandedItems, fTopIndex, nameSpace, gc);

        // draw the foreground markers
        drawMarkers(bounds, fTimeProvider, fMarkers, true, nameSpace, gc);

        // draw the links (arrows)
        drawLinks(boundsWithPadding, fTimeProvider, fItemData.fLinks, nameSpace, gc);

        fTimeGraphProvider.postDrawControl(bounds, gc);

        if (fTimeSelectionEnabled) {
            drawSelection(bounds, gc, nameSpace);
        }
    }

    private void drawSelection(Rectangle bounds, GC gc, int nameSpace) {
     ...
     * @param boundsWithPadding
     *          bounds where content should be displayed
     * @param nameSpace
     ...
     * @since 2.0
     */
    protected void drawBackground(Rectangle bounds, Rectangle boundsWithPadding, int nameSpace, GC gc) {
        ...
        drawEntryNamesAndMiddleLine(boundsWithPadding, nameSpace, gc);
    }

    private void drawEntryNamesAndMiddleLine(Rectangle boundsWithPadding, int nameSpace, GC gc) {
        for (int i = fTopIndex; i < fItemData.fExpandedItems.length; i++) {
            Rectangle itemRect = getItemRect(boundsWithPadding, i);
            if (itemRect.y >= boundsWithPadding.y + boundsWithPadding.height) {
                break;
            }
            Item item = fItemData.fExpandedItems[i];
    ...
    private void updateCursor(int x, int stateMask) {
        ...
        if (fDragState == DRAG_SPLIT_LINE) {
        } else if (fDragState == DRAG_SELECTION && fTimeSelectionEnabled) {
            ...
            cursor = fResizeCursor;
        } else if (!isOverSplitLine(x) && fTimeSelectionEnabled) {
            long selectionBegin = fTimeProvider.getSelectionBegin();
            long selectionEnd = fTimeProvider.getSelectionEnd();
    ...
    /**
     * @param topPadding padding in pixels that is added between the
     * time scale and the graph
     */
    public void setGraphTopPadding(int topPadding)
    {
        fTopPadding = topPadding;
    }

    /**
     * Set if it should be possible to select time range and time point in the graph. By default the option is enabled
     *
     * @param enabled If true time selection will be rendered
     */
    public void setTimeSelectionEnabled(boolean enabled)
    {
        fTimeSelectionEnabled = enabled;
    }
}
